...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        envy = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, envy)
    elif isinstance(procedure, MuProcedure):
        envy = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, envy)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    args_python = []
    if args != nil:
        args_python.append(args.first)
        while args.second != nil:
            args = args.second
            args_python.append(args.first)
    if procedure.use_env == True:
        args_python.append(env)
    try:
        return procedure.fn(*args_python)
    except TypeError: 
        raise SchemeError("I'm not GOD! Check your arguments Bro.")
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            while i < len(formals):
                ...
                i += 1
            return frame
        raise SchemeError('Size Matters!')
...
def do_mu_form(vals):
    ...
    formals = vals[0]
    check_formals(formals) 
    expression = vals.second.first
    if len(vals) > 2:
        expression = Pair('begin', vals.second)
    return MuProcedure(formals, expression)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        if type(vals.second.first) == Pair:
            env.bindings[vals.first] = scheme_eval(vals.second.first, env)
        else:
            env.bindings[vals.first] = vals.second.first
        return target
    elif isinstance(target, Pair):
        if scheme_symbolp(target[0]) is True:
            env.bindings[target[0]] = do_lambda_form(Pair(target.second, vals.second), env)
            return target[0]
        else:
            raise SchemeError("No!")
    else:
...
def do_let_form(vals, env):
    ...
    for key in bindings:
        if scheme_symbolp(key.first) and len(key) < 3:
            names = Pair(key.first, names)
            values = Pair(scheme_eval(key.second.first, env), values)
        else:
            raise SchemeError("One binding please!")
...
def do_and_form(vals, env):
    ...
    i = 0
    while i < (len(vals) - 1):
        if scheme_eval(vals[i], env) is False:
            return False   
        i += 1
...
def check_formals(formals):
    ...
    for symbol in formals:
        if symbol in list_formals:
            raise SchemeError('Why 2 Symbols???')
        if not scheme_symbolp(symbol):
            raise SchemeError('Bad list')
        list_formals.append(symbol)
