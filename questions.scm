...
(define (list-partitions total max-pieces max-value)
  (define (helper total max-pieces max-value partitions_lst)
    (cond [(eq? total 0) (list partitions_lst)]
          [(eq? max-pieces 0) '()] 
          [(eq? max-value 0) '()]
          [else (append (helper (- total max-value) (- max-pieces 1) max-value (append partitions_lst (list max-value))) (helper total max-pieces (- max-value 1) partitions_lst))]))
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               (params (cadr expr))
               (body   (cddr expr)))
              (cond [(eq? (length body) 1) (list form params (car (analyze body)))]
                    [else (list form params (car (apply-to-all analyze body)) (cadr (apply-to-all analyze body)))])
           ))
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
         (cons (cons 'lambda (cons (car (zip values)) (cons (car (apply-to-all analyze body)) nil))) (apply-to-all analyze (car (cdr (zip values)))))
           ))
        (else
